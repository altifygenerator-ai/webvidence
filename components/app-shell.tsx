...
export function AppShell({
  ...
  useEffect(() => {
    ...
    const cookieValue = document.cookie
      .split("; ")
      .find((value) => value.startsWith(`${TIMEZONE_OFFSET_COOKIE}=`))
      ?.split("=")[1];
    ...
    void fetch(`/api/leads/attention-count?tzOffset=${offset}`, {
      cache: "no-store",
      signal: controller.signal,
    })
      .then(async (response) => (response.ok ? response.json() : { count: 0 }))
      .then((data) => setPipelineActionCount(Number(data.count || 0)))
      .catch((error: unknown) => {
  ...
  const navLabel = (label: string) => (
    <span>
      {label}
      {label === "Pipeline" && pipelineActionCount > 0 ? (
        <b
          className="nav-attention-badge"
          aria-label={`${pipelineActionCount} pipeline actions due`}
        >
          {pipelineActionCount}
        </b>
      ) : null}
  ...
  return (
    <div className="app-frame">
      <aside className="app-sidebar">
        <div>
          <Link className="wordmark app-wordmark" href="/dashboard">
            <span>WEB</span>
            <i>V</i>
            <span>IDENCE</span>
          </Link>
        ...
        <div className="sidebar-foot">
          <div className="workspace-status">
            <span className="live-dot" /> Protected workspace
          </div>
          <small>
            Searches, audits, outreach drafts, and plan access are checked on
            the server.
          </small>
      ...
      <header className="mobile-app-header">
        <Link className="wordmark mobile-app-wordmark" href="/dashboard">
          <span>WEB</span>
          <i>V</i>
          <span>IDENCE</span>
        </Link>
      ...
      <main className="app-main">
        <div className="app-topbar">
          <span>WEBVIDENCE OPERATIONS</span>
