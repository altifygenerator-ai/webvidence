...
export default async function Settings({ searchParams }: { searchParams: Promise<{ saved?: string }> }) {
  ...
  const { data: profile } = await db
    ...
    .select('id,name,service_description,typical_project_range,target_customer,outreach_style')
  ...
  return (
    <AppShell admin={user.isAdmin}>
      ...
      <p className="muted settings-intro">These details guide Webvidence when it writes outreach. Verified audit evidence still controls what the message is allowed to claim.</p>
